...
test.describe('browse products', () => {
  async function slugsOnPage(productsPage: ProductsPage) {
    return (
      await productsPage.cards.evaluateAll((cards) => cards.map((c) => c.getAttribute('data-slug')))
    ).sort();
  }
  ...
  test('TC-024 | sorting by price low-to-high orders the visible cards accordingly', async ({
    ...
    const featuredSlugs = await productsPage.cards.evaluateAll((cards) =>
    ...
    await expect(async () => {
      const slugsInOrder = await productsPage.cards.evaluateAll((cards) =>
        cards.map((c) => c.getAttribute('data-slug')),
      );
